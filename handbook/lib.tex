...
\title{ \vspace{-1cm}
	...
	\normalsize for the recently deceased}
...
\definecolor{backcolour}{rgb}{1,0.97,0.95}
...
\subsection{Point location}\label{sub:nie_wiem_co_to_robi} % (fold)
\lstinputlisting[language=C++, firstline=41, lastline=47]{code_snippets/geometry}
% subsection Nie wiem co to robi (end)

\subsection{Point in polygon}\label{sub:point_in_polygon} % (fold)
\lstinputlisting[language=C++, firstline=50, lastline=99]{code_snippets/geometry}
% subsection Nie wiem co to robi (end)

\subsection{Convex hull}\label{sub:convex_hull} % (fold)

Only finds the smallest convex hull.
Points on the hull are omitted.

\lstinputlisting[language=C++, firstline=1, lastline=29]{code_snippets/geometry}
% subsection Nie wiem co to robi (end)
